...
The current safe MVP is read-only. It cannot publish content, upload media,
...
- The CLI exposes only read operations. OAuth includes Yoto's content and icon
  manage scopes because its icon-list endpoint requires them even for reads.
...
- Publishing and player-control requests are refused.
- Nothing generated by an AI is autonomously published for a child.
